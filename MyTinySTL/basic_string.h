...
/*
这个函数调用了 C++ 标准库中的 memcmp 函数，
用于进行字节级别的比较。由于 memcmp 函数是 C 标准库函数，
因此它是用 C 语言编写的，这也就意味着，
这个函数不会抛出任何异常，因此它被标记为 noexcept。
*/
  ...
  /*
  这个函数用于将 src 指向的内存区域中的前 n 个字符复制到 dst 指向的内存区域中，并返回 dst 指向的内存区域。
其中 MYSTL_DEBUG 是一个宏，用于在 debug 模式下进行断言检查。
在这里，它用于检查 src 和 dst 所指的内存区域是否重叠，如果重叠则断言失败。
如果 src 和 dst 所指的内存区域没有重叠，则调用 std::memcpy 函数进行内存拷贝操作。
最后返回 dst 指向的内存区域。*/
...
/*
这段代码是一个重新分配内存空间的函数，其目的是为了满足当前需要的内存空间需求，具体包括以下几个步骤：

首先计算出新的容量大小 new_cap，其中需要的空间量为 need，如果当前容量 cap_ 比 need 小，则将 new_cap 设为 cap_ + (cap_ >> 1)，也就是将当前容量增加一半；否则 new_cap 直接设为 cap_ + need。

然后利用内存分配器 data_allocator 为新的容量分配一块内存空间 new_buffer。

接着使用字符特性（如 std::char_traits）中的 move 函数，将当前内存空间中的内容移动到新的内存空间中，以保留原有数据。

最后释放当前内存空间中的资源，并将 buffer_ 指向新的内存空间，将 cap_ 设为新的容量 new_cap。
*/

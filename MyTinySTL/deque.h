...
// require_capacity 函数
/*这段代码是 deque 容器内部的一个函数，用于确保 deque 中有足够的空间来容纳元素。
参数 n 表示需要的元素个数，front 表示是否是在 deque 前面添加元素。
函数首先判断需要添加元素的位置（前面或后面），然后根据需要计算需要增加的缓冲区数量 need_buffer。
如果需要增加的缓冲区数量超出了当前 map 所能容纳的范围，就需要重新分配一个更大的 map，
并把原来的元素移动到新的 map 中。
如果增加的缓冲区数量在 map 可以容纳的范围内，就调用 create_buffer 函数创建新的缓冲区，
以满足需要添加的元素个数。如果在 deque 的前面添加元素，需要创建的缓冲区应该在 begin_.node 的前面，
否则应该在 end_.node 的后面。*/
template <class T>
...
// reallocate_map_at_front 函数，在分配内存的时候要先计算原内存的二倍大还是需要的内存大
template <class T>
